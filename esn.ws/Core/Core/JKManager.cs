...
namespace JK.Core
{
    ...
    public class JKManager<T> : Paginate where T : class, new()
    {
        ...
        public virtual List<T> GetListEntities(int page = 1, int pageSize = 10, string whereClause = "", string orderBy = "", string order = "ASC")
        {
            ...
            var schema = schemaObj.GetType();
            //get primarykey property
            var pkProp = schema.GetProperty("PrimaryKeyName");
            //get priamry key name
            var pkName = pkProp.GetValue(schemaObj, null).ToString();
            //set orderby default is pkName;
            if (orderBy == "") orderBy = pkName;
